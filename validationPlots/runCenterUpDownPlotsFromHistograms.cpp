...
void runCenterUpDownPlotsFromHistograms()
{
  ...
  std::vector<std::string> vProcesses = {"VBFHToTauTau", "TTToSemiLeptonic", "Embedded-Run2018A-MuTau"};
  ...
  for (unsigned int i = 0; i < vProcesses.size(); i++) {
    ...
    makeShiftedPlots(process, {"CMS_crosstrg_fakefactor"}, {"mutau", "etau"}, {"m_vis"}, "2018", inputDirectory, outputDirectory, "_antiIso");
    ...
    if (containsSubstring(process, "Embedded")) {
      makeShiftedPlots(process, {"CMS_EMB_muES_eta0to1p2", "CMS_EMB_muES_eta1p2to2p1", "CMS_EMB_muES_eta2p1to2p4"}, {"mutau", "emu"}, {"m_vis"}, "2018", inputDirectory, outputDirectory);
      makeShiftedPlots(process, {"CMS_EMB_tauES_dm0", "CMS_EMB_tauES_dm1", "CMS_EMB_tauES_dm10", "CMS_EMB_tauES_dm11"}, {"mutau", "etau"}, {"m_vis"}, "2018", inputDirectory, outputDirectory);
      makeShiftedPlots(process, {"CMS_EMB_trgeff_single", "CMS_EMB_trgeff_cross"}, {"mutau", "etau"}, {"m_vis"}, "2018", inputDirectory, outputDirectory); 
      ...
      makeShiftedPlots(process, {"CMS_EMB_tautrack_dm0dm10", "CMS_EMB_tautrack_dm1", "CMS_EMB_tautrack_dm11"}, {"mutau", "etau"}, {"m_vis"}, "2018", inputDirectory, outputDirectory);  
    }
    else {
      // MC
      makeShiftedPlots(process, {"CMS_muES_eta0to1p2", "CMS_muES_eta1p2to2p1", "CMS_muES_eta2p1to2p4"}, {"mutau", "emu"}, {"m_vis"}, "2018", inputDirectory, outputDirectory);
      makeShiftedPlots(process, {"CMS_tauES_dm0", "CMS_tauES_dm1", "CMS_tauES_dm10", "CMS_tauES_dm11"}, {"mutau", "etau"}, {"m_vis"}, "2018", inputDirectory, outputDirectory);
      ...
      makeShiftedPlots(process, {"CMS_trgeff_single", "CMS_trgeff_cross"}, {"mutau", "etau"}, {"m_vis"}, "2018", inputDirectory, outputDirectory); 
      ...
      if (isMCnonHiggs(process) && !isDY(process)) {
        makeShiftedPlots(process, {"CMS_nonDY"}, {"mutau", "etau", "emu"}, {"m_vis"}, "2018", inputDirectory, outputDirectory);
      }
      if (containsSubstring(process, "TTTo")) {
        makeShiftedPlots(process, {"CMS_toppt"}, {"mutau", "etau", "emu"}, {"m_vis"}, "2018", inputDirectory, outputDirectory);
      }
      ...
      if (doRecoil(process)) {
        makeShiftedPlots(process, {"CMS_met_0j_resolution", "CMS_met_1j_resolution", "CMS_met_gt1j_resolution", "CMS_met_0j_response",  "CMS_met_1j_response",  "CMS_met_gt1j_response"},
         ...
         makeShiftedPlots(process, {"CMS_UES"}, {"mutau", "etau", "emu"}, {"met"}, "2018", inputDirectory, outputDirectory);
      }
    }
  }
