...
void runCenterUpDownPlotsFromHistograms()
{
  ...
  std::vector<std::string> vProcesses = {"ZJ", "ttbar"};
  // "ZJ", "ttbar", "ST", "VV", "WJ", "fake",
  ...
  for (unsigned int i = 0; i < vProcesses.size(); i++) {
    ...
    TString inputFile  = "/eos/cms/store/group/phys_susy/AN-24-166/skkwan/condorHistogramming/2024-11-20-23h57m-benchmark-2018-mutau-iteration1-m_tt/out_mutau.root";
    ...
    std::vector<std::string> defaultVars = {"m_tt", "met"};
    ...
    for (TString category : categories) {
      ...
      makeShiftedPlots(process, category, {"CMS_crosstrg_fakefactor"}, {"mutau", "etau"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory, "_antiIso");

      makeShiftedPlots(process, category, {"CMS_trgeff_Mu8E23_em", "CMS_trgeff_Mu23E12_em", "CMS_trgeff_both_em"},
                                {"emu"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory);

      makeShiftedPlots(process, category, {"CMS_tauidWP_et"},  {"etau"},          {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory);
      // note m_tt_ss here
      makeShiftedPlots(process, category, {"CMS_SScorrection", "CMS_SSclosure", "sCMS_SSboth2D", "CMS_osss"}, {"emu"}, {"m_tt_ss"}, "2018", inputFile, outputDirectoryWithCategory);


      // EMB-specific differences
      if (containsSubstring(process, "Embedded")) {
        makeShiftedPlots(process, category, {"CMS_EMB_muES_eta0to1p2", "CMS_EMB_muES_eta1p2to2p1", "CMS_EMB_muES_eta2p1to2p4"}, {"mutau", "emu"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory);
        makeShiftedPlots(process, category, {"CMS_EMB_tauES_dm0", "CMS_EMB_tauES_dm1", "CMS_EMB_tauES_dm10", "CMS_EMB_tauES_dm11"}, {"mutau", "etau"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory);
        makeShiftedPlots(process, category, {"CMS_EMB_trgeff_single", "CMS_EMB_trgeff_cross"}, {"mutau", "etau"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory); 
        makeShiftedPlots(process, category, {"CMS_EMB_tauideff_pt20to25", "CMS_EMB_tauideff_pt25to30", "CMS_EMB_tauideff_pt30to35", "CMS_EMB_tauideff_pt35to40", "CMS_EMB_tauideff_pt40to500", "CMS_EMB_tauideff_pt500to1000", "CMS_EMB_tauideff_ptgt1000"},
                                {"mutau", "etau"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory);  
        makeShiftedPlots(process, category, {"CMS_EMB_tautrack_dm0dm10", "CMS_EMB_tautrack_dm1", "CMS_EMB_tautrack_dm11"}, {"mutau", "etau"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory);  
      }
      else {
        // MC
        ...
        makeShiftedPlots(process, category, {"CMS_tauES_dm0", "CMS_tauES_dm1", "CMS_tauES_dm10", "CMS_tauES_dm11"}, {"mutau", "etau"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory);
        ...
        makeShiftedPlots(process, category, {"CMS_trgeff_single", "CMS_trgeff_cross"}, {"mutau", "etau"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory); 
        ...
        makeShiftedPlots(process, category, {"CMS_btagsf_hf", "CMS_btagsf_lf", "CMS_btagsf_hfstats1", "CMS_btagsf_hfstats2", "CMS_btagsf_lfstats1", "CMS_btagsf_lfstats2", "CMS_btagsf_cferr1", "CMS_btagsf_cferr2"},
                                {"mutau", "etau", "emu"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory); 
        ...
        makeShiftedPlots(process, category, {"CMS_jetFR_pt0to25", "CMS_jetFR_pt25to30", "CMS_jetFR_pt30to35", "CMS_jetFR_pt35to40", "CMS_jetFR_pt40to50", "CMS_jetFR_pt50to60", "CMS_jetFR_ptgt60"},
                                 {"mutau", "etau"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory, "_antiIso");
    

        // Still MC, but process-specific shifts
        if (isMCnonHiggs(process) && !isDY(process)) {
          makeShiftedPlots(process, category, {"CMS_nonDY"}, {"mutau", "etau", "emu"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory);
        }
        if (containsSubstring(process, "TTTo")) {
          makeShiftedPlots(process, category, {"CMS_toppt"}, {"mutau", "etau", "emu"}, {"m_tt"}, "2018", inputFile, outputDirectoryWithCategory);
        }
