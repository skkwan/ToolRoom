...
os.system("rm hasHPS.txt")
os.system("rm hasNonHPSonly.txt")

for file in files:
    cmdOpen="root -b -q 'macroGetBranches.cpp(\"root://cmsxrootd.hep.wisc.edu/{}\")' | tee temp.txt".format(file)
    print(cmdOpen)
    os.system(cmdOpen)
    cmdFindHPS='grep "HLT_Ele24_eta2p1_WPTight_Gsf_LooseChargedIsoPFTauHPS30_eta2p1_CrossL1" temp.txt'
    cmdFindNon='grep "HLT_Ele24_eta2p1_WPTight_Gsf_LooseChargedIsoPFTau30_eta2p1_CrossL1" temp.txt'
    statusHPS, outputHPS = subprocess.getstatusoutput(cmdFindHPS)
    statusNon, outputNon = subprocess.getstatusoutput(cmdFindNon)
    if (outputHPS != ""):
        print("Found HPS!")
        with open("hasHPS.txt", "a") as f:
            f.write("root://cmsxrootd.hep.wisc.edu/{}\n".format(file))
    else:
        print("Couldn't find HPS")
        if (outputNon != ""):
            print("Found non-HPS")
            with open("hasNonHPSonly.txt", "a") as f:
                f.write("root://cmsxrootd.hep.wisc.edu/{}\n".format(file))
        else:
            print("Couldn't find non-HPS either!")