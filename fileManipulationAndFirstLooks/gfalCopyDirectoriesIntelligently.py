...
for originFolder in originFolders:
    if (originFolder not in existingDestinationFolders):
        ...
        # command = "nohup gfal-copy -p -r -t 999999999 /eos/user/s/skkwan/hToAA/condor/May-21-2022-02h23m-DataMC2018_withGen/%s davs://cmsxrootd.hep.wisc.edu:1094/store/user/skkwan/haabbtt_mt2018/%s > logRun_%s &" % (originFolder, originFolder, originFolder)
        # TEMP: force copy recoil correction ntuples
        command = "nohup gfal-copy -p -r -f -t 999999999 /eos/user/s/skkwan/hToAA/condor/Jun-16-2022-15h37m/%s davs://cmsxrootd.hep.wisc.edu:1094/store/user/skkwan/haabbtt_mt2018/%s > logRun_%s &" % (originFolder, originFolder, originFolder)  
        if (args.execute):
